...
const tag = message.guild.members.cache.filter(m => m.user.username.includes(ayarlar.normal)).size
const tags = message.guild.members.cache.filter(m => m.user.username.includes(ayarlar.yedek)).size
const etikettag = message.guild.members.cache.filter(x => x.user.discriminator.includes("2071")).size
...
\`>\` Sunucumuzun etiket tagını almış ${etikettag.toString().split("").map(a => client.emojis.cache.get(arwSayılar[a])).join("")} adet üye bulunmaktadır.
